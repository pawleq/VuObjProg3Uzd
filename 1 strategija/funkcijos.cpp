...
vector<studentas> vargsiukas;
	...
	{
		failas1 << setw(Pilgis + 6) << std::left << setfill(' ') << kietekas[i].pavarde << setw(Vilgis + 6) << std::left << setfill(' ') << kietekas[i].vardas << setw(16) << std::left << setfill(' ') << std::setprecision(2) << std::fixed << kietekas[i].galutinis << kietekas[i].galutmed << endl;
	}
}
